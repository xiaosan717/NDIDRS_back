...
public class DashboardController {
    ...
    public Result<Map<String, Object>> getStatusDistribution() {
        ...
        List<DormCheckRecord> allRecords = checkRecordService.list(wrapper);

        Map<Long, DormCheckRecord> latestByStudent = new HashMap<>();
        for (DormCheckRecord record : allRecords) {
            DormCheckRecord existing = latestByStudent.get(record.getStudentId());
            if (existing == null || record.getCheckDate().isAfter(existing.getCheckDate())) {
                latestByStudent.put(record.getStudentId(), record);
            }
        }

        long inRoom = 0, leave = 0, late = 0, absent = 0;
        for (DormCheckRecord record : latestByStudent.values()) {
        ...
        distribution.put("total", inRoom + leave + late + absent);
